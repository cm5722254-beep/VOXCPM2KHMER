...
export const Step5Generating: React.FC<Step5GeneratingProps> = ({
  ...
  return (
    <div className="flex flex-col h-full items-center justify-center p-6 md:p-10 relative overflow-hidden bg-[#05070d]">
      ...
      <div className="w-full max-w-3xl flex flex-col gap-6 relative z-10">
        {/* Do Not Close Banner */}
        <div className="flex items-center gap-3.5 p-3.5 px-4 rounded-2xl bg-amber-500/10 border border-amber-500/25 backdrop-blur-md">
          <div className="w-8 h-8 rounded-xl bg-amber-500/20 text-amber-400 flex items-center justify-center shrink-0">
            <AlertTriangle className="w-4 h-4" />
          </div>
          <div className="min-w-0 flex-1">
            <h4 className="text-xs font-bold text-amber-300">ដំណើរការបញ្ចូលសំឡេង AI កំពុងដំណើរការ</h4>
            <p className="text-[11px] text-amber-400/80 leading-relaxed truncate sm:whitespace-normal">
        ...
        <div className="rounded-3xl border border-white/[0.12] bg-[#090c16]/90 backdrop-blur-xl p-6 sm:p-8 shadow-2xl shadow-black/80 flex flex-col gap-6">
          ...
          <div className="flex flex-col items-center justify-center text-center">
            <div className="relative mb-5">
              {hasDubbedOutput ? (
                ...
                <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-sky-500/20 to-indigo-600/20 flex items-center justify-center border border-sky-400/40 shadow-[0_0_35px_rgba(14,165,233,0.25)] relative">
                  <Wand2 className={`w-9 h-9 text-sky-400 ${isDubbing ? 'animate-pulse' : ''}`} />
                  {isDubbing && (
                    <svg className="absolute inset-0 w-full h-full animate-spin text-sky-400" viewBox="0 0 100 100">
                      <circle
                        cx="50"
                        cy="50"
                        r="46"
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-white mb-1.5 tracking-wide font-ui">
              {hasDubbedOutput ? 'បង្កើតសំឡេងខ្មែរជោគជ័យ!' : 'កំពុងបង្កើតសំឡេងតួអង្គខ្មែរ AI...'}
            </h2>
            <p className="text-xs font-mono text-slate-400 max-w-md truncate px-4 py-1 rounded-full bg-white/[0.04] border border-white/[0.06]">
              {hasDubbedOutput
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs font-bold">
              ...
              <span className="text-sky-300 font-mono text-sm">{hasDubbedOutput ? 100 : dubbingProgress}%</span>
            </div>

            <div className="h-3 rounded-full bg-black/60 overflow-hidden border border-white/10 p-0.5">
              <div
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
            {GENERATION_STEPS.map((stg, i) => {
              ...
              return (
                <div
                  key={stg.id}
                  className={`p-2.5 rounded-xl border transition-all flex items-center gap-2.5 text-xs ${
                    isCurrent
                      ? 'bg-sky-500/15 border-sky-400/40 text-white shadow-md shadow-sky-500/15'
                      : isPassed
                      ? 'bg-emerald-500/[0.08] border-emerald-500/25 text-slate-300'
                ...
                >
                  ...
                  >
                    {isPassed ? <Check className="w-3.5 h-3.5 stroke-[3]" /> : isCurrent ? <Loader2 className="w-3.5 h-3.5 animate-spin text-black" /> : stg.id}
                  </div>
                  <div className="min-w-0 flex-1">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 pt-4 border-t border-white/[0.08]">
            <div className="flex flex-col items-center text-center p-2.5 rounded-xl bg-white/[0.03] border border-white/[0.05]">
              ...
              <span className="text-xs text-slate-200 font-semibold truncate w-full px-1" title={uploadedFile?.filename}>
                {uploadedFile?.originalName || uploadedFile?.filename || 'Movie File'}
              </span>
            </div>

            <div className="flex flex-col items-center text-center p-2.5 rounded-xl bg-white/[0.03] border border-white/[0.05]">
              <UserCircle2 className="w-4 h-4 text-indigo-400 mb-1" />
              <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">Unique Cast</span>
              <span className="text-xs text-slate-200 font-semibold">{uniqueCharsCount || 1} Characters</span>
            </div>

            <div className="flex flex-col items-center text-center p-2.5 rounded-xl bg-white/[0.03] border border-white/[0.05]">
              <AlignLeft className="w-4 h-4 text-emerald-400 mb-1" />
            ...
            <div className="flex flex-col items-center text-center p-2.5 rounded-xl bg-white/[0.03] border border-white/[0.05]">
              ...
              <span className="text-xs text-slate-200 font-semibold">Khmer Stem Mix</span>
            </div>
          </div>
          ...
          {hasDubbedOutput && (
            <button
              onClick={onNext}
              className="w-full py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:brightness-110 text-black font-bold text-xs flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/25 transition-all active:scale-95"
            >
